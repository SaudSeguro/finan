...
if( isset( $_GET['acao'] ) && $_GET['acao'] == 'excluir' ){
	...
	@mysql_query(sprintf("DELETE FROM `parcela_titulo` WHERE (`id_parcela`='%s') LIMIT 1", $idParcela));
	...
	$idParcela = isset( $_GET['idParcela'] ) ? intval( $_GET['idParcela'] ) : '-1';
	@mysql_query(sprintf("UPDATE `parcela_titulo` SET `situacao_parcela`='pg' WHERE (`id_parcela`='%s') LIMIT 1", $idParcela));
...
if(isset($_GET['b'])){
	...
	$sql = "SELECT s.sac_nome, s.sac_telefone, s.sac_cidade, s.sac_estado, p.venc_parcela, p.id_parcela, p.valor_parcela, p.titulo_id, p.situacao_parcela, p.`numero_documento`, p.`situacao_parcela`, e.pass_nome FROM parcela_titulo AS p INNER JOIN titulos AS t on p.titulo_id=t.titulo_id INNER JOIN sacado AS s ON s.sac_id=p.sac_id LEFT JOIN `password` AS e ON e.pass_id=t.id_especialista ";
      ...
      <td colspan="2"><table width="100%" border="0">
        <tr>
          <td height="40">Duplicatas vencidas: R$: <strong><?php 
			...
			$sql = @mysql_query(sprintf("SELECT SUM(valor_parcela) AS soma FROM parcela_titulo WHERE `situacao_parcela` = 'ab' AND `venc_parcela` < '%s'",  date('Y-m-d')));
          ...
          <td>Duplicatas vincendo em menos de 07 dias: R$: <strong><?php 
			...
			$sql = @mysql_query(sprintf("SELECT SUM(valor_parcela) AS soma FROM parcela_titulo WHERE situacao_parcela = 'ab' AND `venc_parcela` >= '%s' AND `venc_parcela` <= '%s' ",  date('Y-m-d'), date('Y-m-d', strtotime("+7 day"))));
